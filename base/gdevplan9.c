...
static WImage* initwriteimage(FILE *f, Rectangle r, char*, int depth, gs_memory_t *mem);
...
static dev_proc_map_rgb_color(plan9_rgb2cmap);
static dev_proc_map_color_rgb(plan9_cmap2rgb);
static dev_proc_open_device(plan9_open);
static dev_proc_close_device(plan9_close);
static dev_proc_print_page(plan9_print_page);
static dev_proc_put_params(plan9_put_params);
static dev_proc_get_params(plan9_get_params);
...
gs_private_st_suffix_add1_final(st_plan9_device, plan9_device,
        "plan9_device", plan9_device_enum_ptrs, plan9_device_reloc_ptrs,
                          gx_device_finalize, st_device_printer, p9color);

static const gx_device_procs plan9_procs =
        prn_color_params_procs(plan9_open, gdev_prn_output_page, plan9_close,
                plan9_rgb2cmap, plan9_cmap2rgb,
                gdev_prn_get_params, gdev_prn_put_params);
/*				plan9_get_params, plan9_put_params);*/

plan9_device far_data gs_plan9_device =
{ prn_device_stype_body(plan9_device, plan9_procs, "plan9",
        &st_plan9_device,
...
plan9_rgb2cmap(gx_device *dev, const gx_color_value rgb[])
{
	int shift;
	plan9_device *idev = (plan9_device*) dev;
 ...
 */
extern void init_p9color(ulong *p9color);	/* in gdevifno.c */
static int
...
//		gs_plan9_device.dither, idev->dither);
	idev->p9color = (ulong *)gs_alloc_bytes(idev->memory, p9color_size, "plan 9 colour cube");
    if(idev->p9color == NULL)
		return_error(gs_error_VMerror);
	init_p9color(idev->p9color);

	return gdev_prn_open(dev);
}

/*
 * plan9_close() is called at the end, once everything
 * is finished.  we have nothing to do.
 */
static int
plan9_close(gx_device *dev)
{
        plan9_device *idev = (plan9_device*) dev;
        int code;

        gs_free_object(dev->memory, idev->p9color, "plan 9 colour cube");
...
plan9_print_page(gx_device_printer *pdev, FILE *f)
{
	...
	if(buf == nil) {
		errprintf(pdev->memory, "out of memory\n");
...
//	printf("plan9_print_page dither %d ldepth %d idither %d\n", dither, ldepth, gs_plan9_device.dither);
	...
	w = initwriteimage(f, rect, chanstr, depth, idev->memory);
	if(w == nil) {
	...
	for(y=0; y<pdev->height; y++) {
		...
		if(writeimageblock(w, p, bpl, idev->memory) == ERROR) {
			gs_free(pdev->memory, buf, gsbpl, 1, "plan9_print_page");
			return_error(gs_error_Fatal);
		}
	}
	if(writeimageblock(w, nil, 0, idev->memory) == ERROR) {
		gs_free(pdev->memory, buf, gsbpl, 1, "plan9_print_page");
		return_error(gs_error_Fatal);
	}
...
static WImage*
initwriteimage(FILE *f, Rectangle r, char *chanstr, int depth, gs_memory_t *mem)
{
	WImage *w;
	int n, bpl;
